...
library(ggplot2)
genero
papa <- table(encuesta_habitos$PAPA)
#Base de datos discriminada por el genero
masculino <- filter(encuesta_habitos,encuesta_habitos$genero=="Masculino", na.rm=TRUE)
femenino <- filter(encuesta_habitos,encuesta_habitos$genero=="Femenino", na.rm=TRUE)
#Tabla de frecuencias de PAPA discriminada por el genero
papaxmasculino <- table(masculino$PAPA)
papaxfemenino <- table(femenino$PAPA)
papaxmasculino
papaxfemenino
```